...
default: $(CONF_H)
	make -C $(KDIR) SUBDIRS=$(PWD) PWD=$(PWD) M=$(PWD) BUILD_PREFIX=$(BUILD_PREFIX) KDIR=$(KDIR) KVERSION=$(KVERSION) modules
.PHONY: default
.SUFFIXES:
...
TESTMAKE = make -C $(KDIR) SUBDIRS=$(PWD)/test PWD=$(PWD)/test M=$(PWD)/test EXTRA_CFLAGS='-Wno-unused $(EXTRA_CFLAGS) $1' modules
